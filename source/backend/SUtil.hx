...
class SUtil
{
	public static function filesExists(?to = ''):Bool {
		if(to != '' && !to.endsWith('/'))
			to += '/';
		if((!FileSystem.exists(to + 'assets') && !FileSystem.exists(to + 'mods') || !FileSystem.exists(to + 'mods') || !FileSystem.exists(to + 'assets')))
			return false;
		else
			return true;
	}
