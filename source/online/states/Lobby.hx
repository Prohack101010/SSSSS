    ...
    override function update(elapsed) {
		...
		if (!inputWait) {
			if (controls.ACCEPT) {
				...
				if (curSelected == 3 || curSelected == 4 || curSelected == 0) {
					#if mobileC FlxG.stage.window.textInputEnabled = true; #end
					if(curSelected != 0)
						inputWait = true;
				}
			}
		...
		#if android
		if(FlxG.android.justPressed.BACK && (inputWait || FlxG.stage.window.textInputEnabled)) {
			inputWait = false;
